...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.OCTO_ZED_DB
  process.env.OCTO_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.OCTO_ZED_DB
    else process.env.OCTO_ZED_DB = previous
    home.mockRestore()
  }
})
