...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OCTO_TEST ?? "")'), {
      env: {
        OCTO_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OCTO_TEST_SHELL"], {
      shell: true,
      env: {
        OCTO_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OCTO_TEST_SHELL=ok")
  })
